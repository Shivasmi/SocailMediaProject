...
def index(request):
    ...
    #suggestions for user will start 
    ...
    if request.method == 'POST':
        ...
        if password == password_confirmation:
            if User.objects.filter(email=email).exists():
                ...
                messages.error(request, 'username already taken, choose anotherusername')
                return redirect('signup')
            else: 
...
def like_post(request):
    ...
    if like_filter == None:
        ...
        new_like.save()
        post.no_of_likes += 1 #increment the likes 
        post.save()
        return redirect ('/')
    else:
        like_filter.delete()
        post.no_of_likes -= 1  #decrement the likes 
        post.save()
        return redirect('/')
